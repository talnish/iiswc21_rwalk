    ...
    int num_workers)
{
    ...
    for(int epoch=1; epoch<=num_epochs; epoch++) {
        ...
        for(auto& batch: *train_data_loader) {
            ...
            auto loss = torch::nn::functional::nll_loss(output, target.to(torch::kCUDA));
    ...
    for (const auto& batch : *test_data_loader) {
        ...
        auto output = net->forward(data);
        auto acc = (torch::exp(output)).argmax(1).eq(targets).sum();
        testing_acc += acc.template item<float>();
    }
    t_testing.Stop();
    PrintStep("[TimingStat] Testing  time (s):", t_testing.Seconds());

    std::cout << "Testing accuracy: " << testing_acc/testing_data_size << std::endl;
